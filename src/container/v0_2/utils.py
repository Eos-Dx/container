...
def create_empty_container(
    ...
    with h5py.File(file_path, "w") as f:
        ...
        f.attrs[schema.ATTR_NX_CLASS] = schema.NX_CLASS_ROOT
...
def verify_container_readable(file_path: Union[str, Path]) -> bool:
    ...
    try:
        with h5py.File(file_path, 'r') as f:
            return (
                schema.ATTR_CONTAINER_TYPE in f.attrs
                and schema.ATTR_SCHEMA_VERSION in f.attrs
                and schema.GROUP_ENTRY in f
            )
    except Exception:
        return False
